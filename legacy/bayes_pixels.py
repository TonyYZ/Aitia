        ...
        for i, child in enumerate(node.children):
            if child.label not in nodeSymbols:  # the child is an element
                if random.random() < 0.5:  # alter element pattern
                    ...
                    newChar = compute.reversedCharMapDyn[tuple(currentPattern)] if isDynamic else compute.reversedCharMap[tuple(currentPattern)]
...
def evaluateTree(currentGuideTree, sourceTrials, verbose=0, showZero=True, fillDyn=False):
    ...
    for sourceSeq in sourceTrials:
        for i, sourceFrame in enumerate(sourceSeq):
            similarityLst.append(np.mean(255 - np.abs(scaleUpNearest(sourceFrame, comparedVideo[0].shape) - comparedVideo[i])) / 255)
...
def main():
    ...
    learnedTree, successIter, bestLikelihood = \
        treeSubstitutionMCMC(sourceTrials, numIterations=100, verbose=1, showTree=True,
                             showImage=True,
