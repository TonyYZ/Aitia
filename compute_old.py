...
charMap = {(0, 0, 0): 'ġ', (1, 0, 0): 'g', (0, 1, 0): 'r', (1, 1, 0): 'v', (0.5, 0.5, 0.5): 'n',
           (0, 0, 1): 'd', (1, 0, 1): 'b', (0, 1, 1): 's', (1, 1, 1): 'ṡ',
           (0, 0): 'h', (1, 0): 'l̇', (0, 1): 'ṫ', (1, 1): 'x'}
...
replaceDict = {'parallel': '並', 'series': '串'}

colorMode = False
colorA = (255, 0, 0)
colorB = (0, 200, 200)
unknownTree = ['parallel', ['series', ['ṡ', 0, 0]], ['series', ['m', 0, 0]]]
unknownTree = ['parallel', ['series', ['ġ', 0, 0]]]
global axes
global fig
global ani
global axPrev
...
def getScore(pixelSums=None, pixelLens=None, flowSums=None, flowAbsSums=None, verbose=False):  # how well the
    ...
    if flowSums is None:
        for i in range(len(pixelSums)):
            ...
            k = 1  # gray image
            print('ratio', ratio, k, 1 - math.pow(ratio - k, 2) / math.pow(k, 2))
    ...
    scores = []
    if flowSums is None:
        referent = hexMap if len(pixelSums) == 3 else biMap
        for hexName in referent.keys():
            ...
            if len(example) == 2 and example[0] == example[1] or \
                    len(example) == 3 and example[0] == example[1] and example[1] == example[2]:
                punished = True
                scores[-1] *= punishment
            if len(example) == 2:
                punished = True
...
def splitImage(pixels, origin, depth=0, flow=None, bothOrient=False, flexible=True, buddhist=False):
    ...
    for orient in [0, 1]:
        ...
        for divI, divNum in enumerate([2, 3]):  # bigram and trigram
            ...
            for j in range(divNum):
                ...
                for k in range(round(division * j), round(division * (j + 1))):
                    pixelDict += pixels[k]
                pixelLens[divI].append(len(pixelDict))
                pixelSums[divI].append(sum(pixelDict))
            results = getScore(pixelSums=pixelSums[divI], pixelLens=pixelLens[divI], verbose=True)
            scores.append(results[0])
            sensations.append(results[1])
            hexagrams.append(results[2])
            for j in range(divNum):
        ...
        if flow is not None:
            ...
            results = getScore(pixelLens=pixelLens[1], flowSums=flowSums, flowAbsSums=flowAbsSums, verbose=True)
            dynScore = results[0]
                ...
                for j, moveDir in enumerate([1, -1]):
                    for divI, divNum in enumerate([2, 3]):
                        ...
                        while True:
                            ...
                            results = getScore(pixelSums=newPixelSums[divI], pixelLens=newPixelLens[divI])
                            newScore = results[0]
                            newSensations[divI] = results[1]
                            hexagram = results[2]
                    ...
                    if flow is not None and i == 0:
                        ...
                        while True:
                            ...
                            results = getScore(pixelLens=newPixelLens[1], flowSums=newFlowSums,
...
def removeVoid(guideTree):
    if guideTree[0] == 'parallel':
        ...
        for branch in newGuideTree[1:]:
            if not all([branch[i][0] == 'ġ' for i in range(1, len(branch))]):
                guideTree.append(branch)
        if len(guideTree) == 1:
            guideTree.append(['series', ['ġ', 0, 0]])
...
def countBranch(tree):
    ...
    if tree[0] == 'series':
        ...
        return len(reversedCharMapDbl[tree[0]])

    return branchNum


def analyzeComposition(tree):
    record = anaCompHelper(tree)
    histogram = {}
    for c in record:
...
def anaCompHelper(tree, record=[]):
    if tree[0] in ['series', 'parallel']:
        for comp in tree[1:]:
            record = anaCompHelper(comp, record)
    else:
        record.append(tree[0])
    return record

def drawGuideTree(tree, precision=1, lastProbDicts=[], layerMode=False, normalize=True, verbose=False,
                  hollowDyn=False, acceleration=1, frameStart=0, frameDiv=1, moveHistDict=None, displayTree=False,
                  reverse=False):
    ...
    width = height = countBranch(tree) * 2
    frameLen = round(width * acceleration / frameDiv * (frameStart + 1)) - round(
        width * acceleration / frameDiv * frameStart)
    ...
    while frameI < frameLen * (1 + frameStart):
        ...
        if displayTree and frameI == round(frameLen * frameStart):  # display as zeroth layer tree
            if reverse:
                exhibitTree = reverseGuideTree(
                    currentTree)  # only needed if the test tree is generated, not handwritten
            else:
    ...
    if layerMode:
        codeMap = [[[] for _ in range(width)] for _ in range(height)]  # code maps covering every frame
    else:
        codeMap = None
    ...
    for frameI, pixels in pixelDict.items():
        ...
        mask = pixels[:, :, 1] == 0
        pixels[mask, 1] = 1
        normPixels = np.clip(pixels[:, :, 0] / pixels[:, :, 1], 0, 1)
        if layerMode:
            for i in range(height):
                for j in range(width):
                    if codeMap[i][j]:
                        for mapLegend in codeMap[i][j]:
                            probDict.setdefault(mapLegend, []).append(normPixels[i, j])

            for key in probDict:
                probDict[key] = np.mean(probDict[key])

            if normalize:
                maxProb = max(probDict.values(), default=0)
                if maxProb > 0:
                    for key in probDict:
...
def guideTreeHelper(tree, origin, width, height, pixelDict, orient=0, pixelMovements=None, needCalc=False,
                    depth=0, codeMap=None, calcSeq=None, precision=1, moveHist={}, bigWidth=-1, bigHeight=-1):
    if tree[0] == 'parallel':
        if tree[-1] == 'filler':
            tree = tree[:-1]
        ...
        if depth == 0:
            pixelMovements = np.zeros((bigHeight, bigWidth, 2), dtype=int)
            for i in range(2):
                for branch in tree[1:]:
                    if len(branch) == 1:
                        continue
                    if i == 0:
                        ...
                        moveHist |= newMoveHist
                # print("pixel movements", pixelMovements, bigWidth, bigHeight)
            # print("new move hist", moveHist)
            return pixelDict, codeMap, moveHist
        else:
            newMoveHist = {}
            for branch in tree[1:]:
                if len(branch) == 1:
        ...
        for j in range(height):
            for k in range(width):
                if tree[0] in ['ż', 'm', 'ċ'] or tree[0] in reversedCharMap:
                    ...
                    XLst, YLst = approxPos(lastPt, pixelMovements, calcSeq, lastStep=closestValid, precision=precision)
                    # print("cur calc seq", calcSeq)
                    if startPt not in newPtLst:
                        # print("adding to new pt lst", startPt, XLst, YLst)
                        newPtLst[startPt] = (XLst, YLst)
                    else:
                        print("already exists", startPt)
                    for frameI in calcSeq:
                        roundY = round(YLst[frameI])
                        roundX = round(XLst[frameI])
                        if tree[0] in ['ż', 'm', 'ċ']:
                            fluctuation = np.random.rand() * reversedCharMapDyn[tree[0]][0] * np.random.choice([1, -1])
                            if 0 <= roundY < bigHeight and 0 <= roundX < bigWidth:
                                pixelDict[frameI][roundY, roundX][0] += fluctuation
                        if tree[0] not in ['ġ'] + list(reversedCharMapDyn.keys()):
                            if 0 <= roundY < bigHeight and 0 <= roundX < bigWidth:
                                # print("frameI", frameI, 'coords', YLst[frameI], XLst[frameI], bigHeight, bigWidth, )
                                pixelDict[frameI][roundY, roundX][1] += 1
                    ...
                    if orient == 1:
                        for j in range(width):
                            ...
                            for frameI in calcSeq:
                                roundY = round(YLst[frameI])
                                roundX = round(XLst[frameI])
                                if 0 <= roundY < bigHeight and \
                                        0 <= roundX < bigWidth:
                                    pixelDict[frameI][roundY, roundX][0] += staticPattern[i]
...
def approxPos(lastPt, pixelMovements, calcSeq, lastStep, precision=1):  # Euler's method
    newX, newY = lastPt
    ...
    if pixelMovements is not None:
        ...
        for i, step in enumerate(calcSeq):
            XLst[lastStep] = newX
            YLst[lastStep] = newY
            exceeds = detectExcess(newX, newY, pixelMovements.shape[1] - 1, pixelMovements.shape[0] - 1)
            if exceeds:
                lastStep = step
                continue
            moveTup = pixelMovements[round(newY), round(newX)]
            # print("move tup", moveTup, newX, newY, step, lastStep)
            deltaNewX = moveTup[1] * (step - lastStep) / precision
            deltaNewY = moveTup[0] * (step - lastStep) / precision
...
def calcLayered(guideTrees, normalize=True, layerMode=True, keyLen=20, reverse=False, needZero=False, verbose=False,
        ...
        for frameStart in range(frameDiv):  # update mirror layer components in every frame division
            for layer in range(minLayer, maxLayer):
                ...
                if layer == 0 and needZero:
                    addition = []
                    for tree in guideTreeLst:
                        print("trree", tree)
                        # tree = cleanGuideTree(tree)
                        print('cleaned', tree)
                        if tree[0] == 'series' and len(tree) == 2 and tree[1][0] == 'parallel':
                            addition.append(tree[1])
                        else:
                            addition.append(tree)
                        print("new tree", addition[-1])
                    zeroLayerTrees += addition
        ...
        if replaceText:
            ...
            if not isinstance(tup[1], list) and tup[1] in replaceDict:
                ...
                if useSymbols:
                    ...
                    labels[tup] = \
                        reversedHexBiMap[reversedCharMapDbl[tup[1][0]]]
                    if len(reversedCharMapDbl[tup[1][0]]) == 3:
...
def playLayered(videos, layerLst=[0], showMode=[True, False, True], isSource=False, mirrorMode=False, extraDir='',
...
def drawParamMatrix(sourceTrees, lenMin=1, precMin=1, accMin=1, lenMax=15, precMax=15, accMax=15):
    for z in range(accMin, accMax):
        gif2dLst = []
        for x in range(lenMin, lenMax):
            gif2dLst.append([])
            for y in range(precMin, precMax):
                videos, layerLst, zeroLayerTrees = calcLayered(sourceTrees, normalize=True, reverse=False,
                                                               needZero=True,
                                                               keyLen=x, verbose=1 > 2, fillDyn=False, precision=y,
                                                               acceleration=z)
                playLayered(videos, layerLst, showMode=[True, False, False], isSource=False, mirrorMode=True,
                            extraDir='matrix ' + str(z), extraName=' - prec:' + str(y) + ' len:' + str(x),
                            noWindow=True)
                gif2dLst[-1].append(
                    './images/results/simulations/matrix ' + str(z) + '/layer 0 - prec:' + str(y) + ' len:' + str(
                        x) + '.gif')
        createGifGrid(gif2dLst, './images/results/simulations/matrix/output' + str(z) + '.gif')


def analyzeImages(filename, start, end, frameJump=1, staticMode=True, dynamicMode=False, colorMode=False, scaleFactor=1):
    ...
    for frameOrd, frameNum in enumerate(range(start, end, frameJump)):
        # image = Image.open(r"./images/frames/umbrella1.jpg")
        # image = Image.open("./images/frames/horse1.jpg")
...
def main():
    ...
    # (1, 0): 'l̇', (0, 1): 'ṫ'
    # sourceTrees = ['parallel', ['series', ['parallel', ['series', ['f', 0, -1]]], ['parallel', ['series', ['l̇', 0, -1], ['ṫ', 1, -1]], ['series', ['r', 1, 0]]], ['parallel', ['series', ['ṡ', 1, 0], ['s', 1, 0]], ['series', ['p', 0, 0]], ['series', ['x', 1, -1]]]], ['series', ['ġ', 0, 0], ['v', 1, 0], ['ġ', 0, 0], ['p', 1, 0], ['parallel', ['series', ['m', 1, -1]]], ['z', 1, -1], ['parallel', ['series', ['l', 0, 0]]]], ['series', ['h', 0, 0], ['p', 1, -1]], ['series', ['z', 0, -1], ['r', 0, 0]]]
    '''
    sourceTrees = ['parallel', ['series', ['parallel', ['series', ['ċ', 0, -1], ['g', 0, -1]],
                                           ['series', ['r', 0, -1], ['ġ', 1, -1]]],
                                ['parallel', ['series', ['h', 1, -1], ['c', 1, 0]], ['series', ['r', 0, 0]]]],
                   ['series',
                    ['parallel', ['series', ['ṫ', 0, -1], ['p', 0, -1]], ['series', ['v', 1, 0], ['b', 1, 0]]],
                    ['parallel', ['series', ['c', 0, 0], ['f', 1, -1]], ['series', ['ġ', 1, 0]]]]]
    
    sourceTrees = ['parallel', ['series', ['ġ', 1, 0]], ['series', ['parallel', ['series', ['c', 1, 0]], ['series', ['ġ', 1, 0]]], ['parallel', ['series', ['t', 1, 0]], ['series', ['ṡ', 1, 0]]]]]
    '''
    tmp = [[['parallel', ['series', ['p', 0, 0]], ['series', ['p', 1, 0]], ['series', ['g', 0, 0], ['h', 0, 0], ['h', 0, 0], ['h', 0, 0]], ['series', ['r', 1, 0]]], ['parallel', ['series', ['p', 1, 0]], ['series', ['p', 0, 0]], ['series', ['r', 0, 0]], ['series', ['h', 0, 0], ['h', 0, 0], ['h', 0, 0], ['d', 1, 0]]], ['parallel', ['series', ['p', 0, 0]], ['series', ['c', 1, 0]], ['series', ['h', 0, 0], ['h', 0, 0], ['h', 0, 0], ['d', 0, 0]], ['series', ['r', 1, 0]]], ['parallel', ['series', ['p', 1, 0]], ['series', ['c', 0, 0]], ['series', ['r', 0, 0]], ['series', ['g', 1, 0], ['h', 0, 0], ['h', 0, 0], ['h', 0, 0]]]]]

    sourceTrees = ['parallel', ['series', ['c', 0, -1]],
               ['series', ['parallel', ['series', ['ṡ', 0, -1]]] + tmp[0][0][1:]] + [branch + [['series', ['ġ', 0, -1]]] for branch in tmp[0][1:]]]
    sourceTrees = [[['parallel', ['series', ['parallel', ['series', ['n', 1, 0]], ['series', ['parallel', ['series', ['r', 1, 0]], ['series', ['n', 0, 0]]], ['parallel', ['series', ['n', 1, 0]], ['series', ['n', 0, 0]]], ['parallel', ['series', ['r', 1, 0]], ['series', ['n', 0, 0]]]]]], ['series', ['parallel', ['series', ['n', 0, 0]], ['series', ['parallel', ['series', ['n', 1, 0]], ['series', ['s', 0, 0]]], ['parallel', ['series', ['n', 1, 0]], ['series', ['r', 0, 0]]], ['parallel', ['series', ['n', 1, 0]], ['series', ['n', 0, 0]]]]]]]]]
    sourceTrees = sourceTrees[0][0]
    sourceTrees = ['parallel', ['series', ['parallel', ['series', ['ġ', 0, 0]]], ['parallel', ['series', ['h', 1, -1]]], ['parallel', ['series', ['z', 1, 0]]]], ['series', ['v', 0, -1]], ['series', ['d', 0, 0]], ['series', ['p', 1, 0]], ['series', ['ṫ', 1, -1]]]

    # sourceTrees = demoBackups.revolve
    # sourceTrees = demoBackups.shortSlideShow
    # drawParamMatrix(sourceTrees, lenMin=1, lenMax=15)
    ...
    videos, layerLst, zeroLayerTrees = calcLayered(sourceTrees, normalize=True, reverse=False, needZero=True, keyLen=4,
                                                   acceleration=2,
                                                   verbose=False, fillDyn=True, mirrorMode=True, precision=3)
    ## !!!reverse when automatically generated, do not reverse when handwritten!!!
    print("videos", videos)
    ...
    res = analyzeImages('line', 0, 21, scaleFactor=5)
    print('res', res)
